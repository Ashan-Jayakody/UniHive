...
const PeerTutoring = () => {
    ...
    switch (name) {
      ...
      case "sessionLink":
        ...
        if (!/^https?:\/\//i.test(trimmed)) {
          return "Session link must begin with http:// or https://.";
        }
        return "";
      ...
      case "maxStudents": {
        ...
        if (!Number.isInteger(count) || count <= 0) {
          return "Maximum student count must be a positive whole number.";
        }
        return "";
      }
  ...
  const validateForm = () => {
    ...
    if (formData.date && formData.time) {
      ...
      if (selectedDateTime < now) {
        nextErrors.time = "The selected time cannot be in the past.";
      }
    }
  ...
  const handleChange = (event) => {
    ...
    if ((name === "time" || name === "endTime") && errors.endTime) {
      setErrors((current) => {
        const next = { ...current };
        delete next.endTime;
        return next;
        ...
        {user?.role === "admin" ? (
            ...
            {adminSessions.length === 0 ? (
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <p className="text-sm text-slate-600">
                  There are no peer tutoring sessions submitted yet.
                </p>
              </div>
            ) : (
          ...
          <>
                  ...
                  <div className="mb-6 flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        All fields are required. The date cannot be in the past and
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="grid gap-5 sm:grid-cols-2">
                      ...
                      <div>
                        ...
                        />
                        {errors.time && (
                          <p className="mt-2 text-xs text-rose-600">{errors.time}</p>
                        )}
                      </div>
                    ...
                    >
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div className="mt-4 flex flex-col gap-3 sm:mt-0 sm:w-72">
                          {session.approvalStatus === "approved" &&
                            getSessionStatus(session.date, session.time, session.endTime) ===
                              "active" && (
                              <button
                                type="button"
                                onClick={() =>
            ...
            <section className="mt-6 space-y-4">
              <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <h2 className="text-lg font-semibold text-slate-900">
                  Available peer tutoring sessions
                </h2>
                <p className="mt-2 text-sm text-slate-500">
                  Only approved sessions are visible to students.
                </p>
              </div>
              ...
              {approvedSessions.length === 0 ? (
                <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                  <p className="text-sm text-slate-600">
                    No approved sessions are available at the moment.
                  </p>
                </div>
              ) : (
                              ...
                              <div className="mt-6 rounded-2xl border border-indigo-100 bg-indigo-50/50 p-5">
                                ...
                                <div className="mt-3 flex gap-2">
                                  <button
                                    ...
                                    disabled={
                                      feedbackLoading || !feedbackFormData.comment.trim()
                                    }
                              ...
                              return (
                                <div className="flex flex-col gap-2">
                                  <button
                                    type="button"
                                    onClick={() => {
                                      setActiveFeedbackSessionId(
                                        session._id === activeFeedbackSessionId
                                          ? null
                                          : session._id
                                      );
                                      setFeedbackFormData({ rating: 5, comment: "" });
          ...
          <div className="w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="flex items-start justify-between border-b border-slate-200 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Participants for {activeParticipantSession.moduleName}
                </p>
              </div>
              <div className="text-right">
                <p className="text-sm font-semibold text-slate-700">Total joined</p>
                <p className="text-2xl font-semibold text-slate-900">
                  {activeParticipantSession.participants?.length || 0}
                </p>
              </div>
            </div>
