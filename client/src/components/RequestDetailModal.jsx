          ...
          {request.attachmentUrl && (
            <div>
              ...
              {request.attachmentUrl.match(/\.(jpeg|jpg|png|gif)$/i) ? (
                <img
                  src={`http://localhost:5000${request.attachmentUrl}`}
                  alt="Attachment"
                  className="max-h-40 rounded-lg border border-slate-200 cursor-pointer hover:opacity-90 transition-opacity"
                  onClick={() => window.open(`http://localhost:5000${request.attachmentUrl}`, '_blank')}
                />
              ) : (
                <a
                  href={`http://localhost:5000${request.attachmentUrl}`}
                  target="_blank"
